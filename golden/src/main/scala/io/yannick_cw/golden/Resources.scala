...
/**
  * Taken from: https://github.com/circe/circe-golden
...
object Resources {
  ...
  lazy val inferRootDir: File = {
    ...
    val resourceDir =
      new File(new File(new File(current.getParentFile, "src"), "test"), "resources")
  ...
  def inferPackage[A](implicit A: TypeTag[A]): List[String] =
    owners(A.tpe)
      .collectFirst { case s if s.isPackage => s.fullName.split('.').toList }
